...
export default async function Banner({ theme = "dark" }: Props) {
  ...
  return (
    <div
      data-theme={theme}
      className="relative overflow-hidden text-sm data-[theme=dark]:text-stone-50 data-[theme=light]:text-stone-800"
    >
      {data && (
        <div
          data-theme={theme}
        ...
        >
          <span>
            {`${data.result.description} `} <strong>Shop now</strong>
            <Link href="/shop">
              <ChevronRight
                className="inline"
                size={20}
