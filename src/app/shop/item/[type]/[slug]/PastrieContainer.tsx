import { getPastrieBySlug } from "@/lib/getItemBySlug";
...
export default async function PastrieContainer({ params }: Props) {
  const data = await getPastrieBySlug(params.type, params.slug);
  ...
  return (
    <>
      ...
      <div className="grid grid-flow-row grid-cols-1 gap-12 px-6 pb-16 pt-8 sm:mx-[var(--global-md-viewport-padding)] sm:px-0 lg:grid-cols-2 lg:grid-rows-1 lg:gap-16 lg:pb-32 xl:gap-32">
        ...
        />
        <PastrieEditor result={data.result} />
      </div>
    </>
  );
}
